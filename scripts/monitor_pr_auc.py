...
FEATURES_LIVE_PATH = Path(os.getenv("FEATURES_LIVE_PATH", "data/processed/features_live.parquet"))
UPDATE_INTERVAL_SECONDS = int(os.getenv("PR_AUC_UPDATE_INTERVAL", "600"))  # 10 minutes default
TIME_WINDOW_HOURS = float(os.getenv("PR_AUC_TIME_WINDOW", "1"))  # Last N hours default (supports fractional hours like 0.5)
LOOKBACK_BUFFER_MINUTES = int(os.getenv("PR_AUC_LOOKBACK_BUFFER", "5"))  # Only use predictions from at least 5 minutes ago (to ensure features exist)
KAFKA_CONSUMER_TIMEOUT_MS = int(os.getenv("KAFKA_CONSUMER_TIMEOUT_MS", "120000"))  # 2 minutes default (increased from 5 seconds)
KAFKA_MAX_MESSAGES = int(os.getenv("KAFKA_MAX_MESSAGES", "50000"))  # Max messages to read per cycle
...
def load_predictions_from_kafka(
    ...
    try:
        ...
        max_time = now - timedelta(minutes=lookback_buffer_minutes)  # Don't use very recent predictions
        ...
        try:
            ...
            if not partitions:
                logger.warning("No partitions assigned after waiting; will read from committed offset")
            else:
                consumer.seek_to_end()
                end_offsets = consumer.end_offsets(partitions)

                messages_to_read = int(time_window_hours * 3600 * 2)  # 2x buffer for safety
            ...
            logger.warning(f"Could not seek in topic: {e}, will read from committed offset")
        ...
        max_consecutive_old = 100  # If we see 100 consecutive old messages, we've passed the window
        error_count = 0  # Surface the first few parsing errors at warning level
        
        # Read messages and filter by time window
        start_time = time.time()
        max_processing_time = min(KAFKA_CONSUMER_TIMEOUT_MS / 1000, 20)  # Max 20 seconds
        ...
        try:
            for message in consumer:
                ...
                if elapsed > max_processing_time:
                    logger.info(f"Reached max processing time ({max_processing_time:.1f}s), stopping read. Processed {processed_count} messages.")
                    break
                ...
                try:
                    ...
                    pred_time = pd.to_datetime(pred.get("timestamp"), utc=True, errors="coerce")
                    ...
                    if pred_time < cutoff_time:
                        ...
                        if consecutive_old_messages > max_consecutive_old and message_count > 0:
                            logger.info(f"Processed {consecutive_old_messages} consecutive old messages, stopping read")
                            break
                        # Also break if we've processed too many old messages total
                        if skipped_old > 2000 and message_count > 0:
                            logger.info(f"Processed {skipped_old} old messages, stopping to avoid timeout")
                            break
                        continue
                    ...
                    log_fn(
                        f"Error processing message {processed_count}: {e}"
                    )
                    continue
...
def match_predictions_with_labels(
    ...
    if "timestamp" not in predictions_df.columns or "timestamp" not in features_df.columns:
    ...
    if len(merged) > 0:
        ...
        if len(predictions_df) > 0 and len(features_df) > 0:
            ...
            if pred_max < feat_min:
                logger.warning(f"Predictions are all before features: pred_max ({pred_max}) < feat_min ({feat_min})")
            elif pred_min > feat_max:
                logger.warning(f"Predictions are all after features: pred_min ({pred_min}) > feat_max ({feat_max})")
            else:
    ...
    try:
        ...
        with mlflow.start_run(run_name=run_name, run_id=run_id):
            ...
            mlflow.log_metrics(
                ...
                step=int(current_time.timestamp()),  # Use timestamp as step for time-series
            )
...
def calculate_and_log_pr_auc():
    ...
    if matched_df is None or len(matched_df) == 0:
        logger.warning("No matched predictions with labels, skipping PR-AUC calculation")
        return
    ...
    model_version = matched_df["model_version"].mode()[0] if "model_version" in matched_df.columns else "production"
...
def main():
    ...
    logger.info(f"Update interval: {UPDATE_INTERVAL_SECONDS} seconds ({UPDATE_INTERVAL_SECONDS/60:.1f} minutes)")
    logger.info(f"Time window: {TIME_WINDOW_HOURS} hours")
    logger.info(f"Lookback buffer: {LOOKBACK_BUFFER_MINUTES} minutes (excludes recent predictions)")
    ...
    logger.info(f"Predictions: Kafka topic {KAFKA_TOPIC_PREDICTIONS} or {PREDICTIONS_LOG_DIR}")
    logger.info("=" * 70)
